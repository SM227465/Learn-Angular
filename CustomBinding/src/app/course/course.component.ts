...
export class CourseComponent implements OnInit {
  ...
  getFreeCourses() {
    const freeCourses = this.courses.filter((course) => course.type === 'Free');
    return freeCourses.length;
  }

  getPremiumCourses() {
    const premiumCourses = this.courses.filter(
      (course) => course.type === 'Premium'
    );
    return premiumCourses.length;
  }
